...
new_gt_path  =  video_dir+"gt/new_gt.txt"
new_gt = []
...
for i, p in enumerate(pId):
    ...
    #     cv2.waitKey(1)
    for j in range(pData.shape[0]):
        if pData.shape[0] > 200 and j %10==0:
            new_gt.append(pData[j])
print(np.array(new_gt).shape)
np.savetxt(new_gt_path, np.array(new_gt), delimiter=",")
